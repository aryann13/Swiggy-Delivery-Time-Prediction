...
if __name__ == "__main__":
    ...
    with open(params_file_path, "r") as f:
        all_params = yaml.safe_load(f)["Train"]
    ...
    with mlflow.start_run() as run:
        # set tags
        mlflow.set_tag("model", "Stacking Regressor - RF + XGBoost")

        # log parameters (RF and XGBoost individually — model.get_params() fails for nested models)
        ...
        # log the final model (skops_trusted_types needed because XGBoost is in the stacking pipeline)
        TRUSTED_TYPES = [
        ...
        try:
            mlflow.sklearn.log_model(
                model, "delivery_time_pred_model",
                signature=model_signature,
                skops_trusted_types=TRUSTED_TYPES,
            )
        except Exception as e:
            logger.warning(f"mlflow.sklearn.log_model error: {e}. Falling back to log_artifact")
            mlflow.log_artifact(root_path / "models" / "model.joblib", artifact_path="delivery_time_pred_model")
